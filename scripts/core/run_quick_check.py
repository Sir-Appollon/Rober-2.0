...
try:
    ...
    print(
        f"[DEBUG - run_quick_check.py - PLEX - 3] Active Plex sessions: {session_count}"
    )
    ...
    for session in sessions:
        try:
            ...
            video_decision = (
                session.videoDecision if hasattr(session, "videoDecision") else "N/A"
            )
            ...
            print(
                f"[DEBUG - run_quick_check.py - PLEX - 4] {user} watching {media_title} on {player} - {video_decision}, {video_codec}/{audio_codec}, {resolution}, {duration}s"
            )
            ...
            print(
                f"[DEBUG - run_quick_check.py - PLEX - Error] Transcode check error: {e}"
            )
    ...
    if EXTERNAL_PLEX_URL:
        try:
            response_ext = subprocess.run(
                ["curl", "-f", "-s", "--max-time", "5", f"{EXTERNAL_PLEX_URL}/web"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
        ...
        plex_msg_lines.append(
            f"[EXTERNAL ACCESS] Plex accessible externally: {external_accessible}"
        )
    else:
