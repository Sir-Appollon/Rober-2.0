...
for discord_path in discord_paths:
    if os.path.isfile(discord_path):
        ...
        try:
            ...
            print(
                f"[DEBUG - run_quick_check.py - DISCORD - Error] Failed to import module: {e}"
            )
    else:
        print(
            f"[DEBUG - run_quick_check.py - DISCORD - Missing] File not found: {discord_path}"
        )

if not send_discord_message:
    print(
        "[DEBUG - run_quick_check.py - DISCORD - FAIL] Could not load Discord notifier module."
    )
...
def get_deluge_ip():
    ...
    try:
        ...
        raise RuntimeError(
            f"[DEBUG - get_deluge_ip - fallback] Could not detect IP inside deluge container: {e}"
        )
...
print(
    "[DEBUG - run_quick_check.py - NETWORK - 2] Internet access and speed test from Deluge"
)
try:
...
try:
    ...
    plex_msg_lines.append(
        f"[SPEEDTEST] Download: {download_speed:.2f} Mbps | Upload: {upload_speed:.2f} Mbps"
    )
except:
...
try:
    ...
    for session in sessions:
        try:
            ...
            resolution = (
                f"{session.media[0].videoResolution}p" if session.media else "N/A"
            )
            duration = (
                int(session.viewOffset / 1000) if hasattr(session, "viewOffset") else 0
            )
            transcode_active = (
            ...
            plex_msg_lines.append(
                f"[SESSION] {user} watching {media_title} on {player}"
            )
            plex_msg_lines.append(
            ...
            print(
                f"[DEBUG - run_quick_check.py - PLEX - Error] Session parse error: {e}"
            )
    ...
    # Test external access via HTTPS API call
    if EXTERNAL_PLEX_URL:
        try:
            url = f"{EXTERNAL_PLEX_URL}/status/sessions?X-Plex-Token={PLEX_TOKEN}"
            resp = requests.get(url, timeout=5)
            if resp.status_code == 200:
                plex_msg_lines.append(
                    "[EXTERNAL ACCESS] Plex public URL accessible (API OK)"
                )
            else:
                plex_msg_lines.append(
                    f"[EXTERNAL ACCESS] Plex public URL responded, status {resp.status_code}"
                )
        except Exception as e:
            plex_msg_lines.append(
                f"[EXTERNAL ACCESS] Plex public URL NOT accessible: {e}"
            )
    else:
        plex_msg_lines.append("[EXTERNAL ACCESS] No external DOMAIN configured")
    ...
    try:
        ...
        plex_msg_lines.append(
            f"[PROCESS] Plex CPU usage (Docker): {cpu:.2f}% | RAM usage: {mem:.2f}%"
        )
    except Exception as e:
        plex_msg_lines.append(
            f"[PROCESS] Failed to get Plex usage via docker stats: {e}"
        )

    # Disk usage of transcode
    ...
    try:
        if os.path.exists(TRANSCODE_PATH):
            ...
            plex_msg_lines.append(
                f"[DISK] Transcode folder not found at {TRANSCODE_PATH}"
            )
    except Exception as e:
    ...
    # System stats
    plex_msg_lines.append(
        "[DEBUG - run_quick_check.py - SYSTEM - 1] Gathering system stats"
    )
    ...
    plex_msg_lines.append(
        f"[SYSTEM] Internet I/O - Sent: {net_io.bytes_sent / (1024**2):.2f} MB | Received: {net_io.bytes_recv / (1024**2):.2f} MB"
    )
    plex_msg_lines.append(
        f"[SYSTEM] Disk I/O - Read: {disk_io.read_bytes / (1024**2):.2f} MB | Write: {disk_io.write_bytes / (1024**2):.2f} MB"
    )
    ...
    plex_msg_lines.append(f"[ERROR] Plex test failed: {e}")

if discord_connected:
    print(
        "[DEBUG - run_quick_check.py - DISCORD - SUCCESS] Discord message sent successfully"
    )
else:
...
def get_deluge_stats():
    ...
    try:
        ...
        stats["download_rate"] = round(
            session_stats[b"download_rate"] / 1024, 2
        )  # KB/s
...
try:
    ...
    data_entry = {
        ...
        "network": {
            "vpn_ip": (
                [vpn_ip_pub, vpn_ip_int]
                if "vpn_ip_pub" in locals() and "vpn_ip_int" in locals()
                else []
            ),
            "deluge_ip": (
                [deluge_ip_pub, deluge_ip_int]
                if "deluge_ip_pub" in locals() and "deluge_ip_int" in locals()
                else []
            ),
            "internet_access": (
                internet_check.returncode == 0
                if "internet_check" in locals()
                else False
            ),
            "speedtest": {
                "download_mbps": (
                    round(download_speed, 2) if "download_speed" in locals() else 0.0
                ),
                "upload_mbps": (
                    round(upload_speed, 2) if "upload_speed" in locals() else 0.0
                ),
            },
        },
        "plex": {
            ...
            "unique_clients": (
                len(users_connected) if "users_connected" in locals() else 0
            ),
            "transcoding_sessions": (
                transcode_count if "transcode_count" in locals() else 0
            ),
        ...
        "system": {
            ...
            "disk_io": {
                "read_mb": (
                    round(disk_io.read_bytes / (1024**2), 2)
                    if "disk_io" in locals()
                    else 0.0
                ),
                "write_mb": (
                    round(disk_io.write_bytes / (1024**2), 2)
                    if "disk_io" in locals()
                    else 0.0
                ),
            },
        ...
        "deluge": {
            ...
            "download_rate_kbps": (
                deluge_stats["download_rate"] if deluge_stats else 0.0
            ),
