...
def check_deluge_activity():
    try:
        ...
        for torrent in torrents.values():
            if torrent[b"state"] in (b"Downloading", b"Seeding"):
                return True
        return False
    except:
        return False
...
def get_docker_uptimes():
    try:
        out = subprocess.check_output("docker ps --format '{{.Names}} {{.RunningFor}}'",
                                      shell=True).decode().strip()
        return out
    except:
        return "Unavailable"

# Start SEV detection logic
if not check_plex_local():
    ...
    subprocess.run(["python3", "/app/scripts/SEV/Ident/sev0.py"])
    print("FAILURE")
    ...
    subprocess.run(["python3", "/app/scripts/SEV/Ident/sev1.py"])
    print("FAILURE")
