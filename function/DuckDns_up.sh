...
ENV_FILE="$(dirname "$0")/../.env"  # Assumes .env is at project root
if [[ -f "$ENV_FILE" ]]; then
...
# 3. Get IP from DuckDNS record
DUCK_IP=$(dig +short "$DUCKDNS_DOMAIN")
...
if [[ "$CURRENT_IP" == "$DUCK_IP" ]]; then
    ...
    if [[ "$REPLY" =~ ^[Yy]$ ]]; then
        ...
        curl -s "https://www.duckdns.org/update?domains=$DUCKDNS_DOMAIN&token=$DUCKDNS_TOKEN&ip=" -o ~/duckdns.log
